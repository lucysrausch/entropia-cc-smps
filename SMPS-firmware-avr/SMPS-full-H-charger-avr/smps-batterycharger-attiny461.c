...
/* Start A/D to get the battery voltage before we start.
Wait for the analogue electronics to settle. */
    do
    {
...
/* Start A/D for first conversion (battery current feedback) every cycle */
        outb(ADMUX,0xE5);           /* MUX AD5, reference 2.56V, left adjust */
        outb(ADCSRB,0x10);          /* Higher gain plus REFS2 and mux bits*/
        outb(ADCSRA,0xC3);          /* Start Conversion, clock divider 8 (1MHz) */
        while ((inb(ADCSRA) & _BV(ADSC)) > 0);  /* poll busy on ADC */
        ibatt = inb(ADCH);
/* Start A/D for second conversion (load current feedback) every 4 cycles */
        if ((counter & 0x03) == 0)
        {
...
/* Start A/D for third conversion (input voltage feedforward) every 4 cycles */
        if ((counter & 0x03) == 1)
        {
...
/* Start A/D for fourth conversion (output voltage feedback) every 4 cycles */
        if ((counter & 0x03) == 2)
        {
            outb(ADMUX,0xE4);       /* MUX ADC4, Reference 2.56V, left adjust */
            outb(ADCSRB,0x10);      /* Additional REFS2 bit */
...
/* SMPS */
/* buck-boost control algorithm with voltage/current limits */
...
/* CONTROL ALGORITHMS */
        ...
        if ((counter & 0x03) == 3)     /* Algorithms updated on the fourth cycle */
        {
...
The filter coefficient is a power of 8 to facilitate multiply and divide operations.
...
/* CHARGER ALGORITHM STARTUP PHASE */
...
Don't do this if the load has triggered the charger off, otherwise it comes back on. */
            ...
            if (! triggered)
            {
                if ((vbattAvg > vbattOriginal) && (ibattAvg > currentTrigger))
                {
...
voltage target to 2.3V per cell. Start checking after startup has settled. */
            if ((counter > 0xEFFF) && (ibattAvg <= floatTrigger))
            {
...
This step is done to prevent the charger interfering with sensitive equipment. */
            if ((iloadAvg > loadTrigHi) && ((counter & 0x1F) == 0))
            {
                if (! triggered)
                {
                    ...
                    sbi(PORTB,6);       /* signal on spare port */
                }
                triggered = TRUE;
            }
            if ((iloadAvg < loadTrigLo) && ((counter & 0x1F) == 0))
            {
                if (triggered)
                {
                    outb(TCCR1A,0x11);  /* OC1A not used, OC1B as PWM with both outputs */
                    ...
                    counter = 0;        /* Prevent excessive switching */
                    cbi(PORTB,6);       /* signal on spare port */
                }
                triggered = FALSE;
            }
        }
