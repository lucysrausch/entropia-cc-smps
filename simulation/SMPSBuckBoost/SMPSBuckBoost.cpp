...
// Target output voltage and current
	...
	double C = 220;		// Filter capacitor uF
	double R = 6;		// Load resistor ohms
	int buckCycle = 14;	// Length of s1 pulse
	...
	std::cout << std::endl;
	std::cout << "Time" << "," << "Switch 1" << "," << "Switch 2" << "," << "Inductor Current" << "," << "Diode Current" << "," << "Switch Current" << "," << "Load Current";
	std::cout << std::endl;
...
// The step time is 0.16us, so this is done every 26/0.16=160us.
        if ((i % 160) == 0)
        {
			if ((Vc < voltage) && (Ir < current))
			{
				if (buckCycle < buckmax) buckCycle++;
				else if ((boostCycle < boostmax) || ((i % 4096) == 0)) boostCycle++;
			}
			else
			{
				if (boostCycle > 0) boostCycle--;
				else if (buckCycle < 4) buckCycle--;
			}
        }

// The time starts with all variables at zero and s1 turns on with s2 off. After delay
// delay s2 turns on. At the end of duty cycle buckCycle s1 turns off. At the end of d+boostCycle s2 turns
// off. The program must be able to switch these in any specified order.
...
// Source is isolated and inductor current flows into the capacitor.
		...
		std::cout << t << "," << s1 << "," << s2 << "," << Il << "," << Id1 << "," << Is2 << "," << Ir << "," << Vc;
		std::cout << std::endl;
	}
}
